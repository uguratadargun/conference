...
export const setupRoomEventHandlers = (
  ...
  room.on(RoomEvent.TrackUnsubscribed, (track, _publication, participant) => {
    ...
    if (track.kind === Track.Kind.Audio) {
      ...
      try {
        track.detach();
        console.log(
          `🔇 Audio track properly detached for ${participant.identity}`
        );
      } catch (error) {
        console.warn(
          `Failed to detach audio track for ${participant.identity}:`,
          error
        );
      }
  ...
  room.on(RoomEvent.AudioPlaybackStatusChanged, () => {
    console.log(
      "Audio playback status changed. Can play audio:",
  ...
  room.on(RoomEvent.Connected, async () => {
    ...
    if (room.canPlaybackAudio !== undefined && !room.canPlaybackAudio) {
      console.log(
        "⚠️ Audio playback is restricted, will start on user interaction"
      );
    } else {
  ...
  room.on(
    ...
    (quality: ConnectionQuality, participant) => {
      if (participant.isLocal) {
        console.log(
          `🔄 Connection quality changed for local participant: ${quality}`
        );
  ...
  room.on(
    ...
    (_prevPermissions: any, participant: any) => {
      setRoomState((prev) => ({
        ...prev,
        participants: prev.participants.map((p) =>
          p.id === participant.identity
  ...
  room.on(RoomEvent.TrackSubscriptionFailed, (_trackSid, participant) => {
    ...
    setRoomState((prev) => ({
      ...prev,
      participants: prev.participants.map((p) =>
        p.id === participant.identity ? { ...p, hasConnectionIssues: true } : p
      ),
    }));
  });
  ...
  room.on(
    ...
    (_pub, streamState, participant) => {
      setRoomState((prev) => ({
        ...prev,
        participants: prev.participants.map((p) =>
          p.id === participant.identity ? { ...p, streamState: streamState } : p
        ),
      }));
    }
  );
