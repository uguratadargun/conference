...
import {
  GRID_LAYOUTS,
  expandAndSortLayoutDefinitions,
  selectGridLayout,
} from "./grid-layouts";

describe.concurrent(
  "Test correct layout for participant count with no screen size limits:",
  () => {
...
describe.concurrent(
  ...
  () => {
    test.each([
      {
        desiredGrid: { columns: 4, rows: 4 },
        expected: { columns: 3, rows: 3 },
      },
      {
        desiredGrid: { columns: 5, rows: 5 },
        expected: { columns: 4, rows: 4 },
      },
      {
        desiredGrid: { columns: 3, rows: 3 },
      ...
      ({ desiredGrid, expected }) => {
        const desiredLayout = expandAndSortLayoutDefinitions(GRID_LAYOUTS).find(
          (layout_) =>
            layout_.columns === desiredGrid.columns &&
            layout_.rows === desiredGrid.rows,
        );
        if (desiredLayout === undefined)
          throw new Error("Could not find the desired layout.");
